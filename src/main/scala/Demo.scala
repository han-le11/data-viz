...
object Demo {
  def main(args: Array[String]): Unit = {
    ...
    val open = new Line("Open", (xAxis zip (data("Open").map(x => x.toDouble))).takeRight(30))
    demoGraph.addInput(open)
    println(open.data)

    val close = new Line("Close", (xAxis zip data("Close").map(x => x.toDouble)).takeRight(30))
    demoGraph.addInput(close)

    val high = new Line("High", (xAxis zip data("High").map(x => x.toDouble)).takeRight(30))
    demoGraph.addInput(high)

    val low = new Line("Low", (xAxis zip data("Low").map(x => x.toDouble)).takeRight(30))
